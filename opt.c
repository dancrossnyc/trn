...
opt_init (int argc, char *argv[], char **tcbufptr)
{
    ...
    memset((char *)option_def_vals, 0, INI_LEN(options_ini) * sizeof(char *));
    ...
    memset((char *)option_saved_vals, 0,
           INI_LEN(options_ini) * sizeof(char *));
    option_flags = (char*)safemalloc(INI_LEN(options_ini)*sizeof(char));
    memset(option_flags, 0, INI_LEN(options_ini) * sizeof(char));
...
void
opt_file (char *filename, char **tcbufptr, bool bleat)
{
...
void
set_header (char *s, int flag, bool setit)
{
    int i;
    int len = strlen(s);
    for (i = HEAD_FIRST; i < HEAD_LAST; i++) {
