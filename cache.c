...
check_poster (ARTICLE *ap)
{
    if (auto_select_postings && (ap->flags & AF_EXISTS) && ap->from) {
        ...
        if (!(ap->flags & AF_FROMTRUNCED)) {
            ...
            char* h;
            strcpy(s,ap->from);
            if ((h=strchr(s, '<')) != NULL) { /* grab the good part */
                s = h+1;
                if ((h=strchr(s, '>')) != NULL)
                    *h = '\0';
            } else if ((h=strchr(s, '(')) != NULL) {
                while (h-- != s && *h == ' ')
                    ;
                h[1] = '\0';            /* or strip the comment */
            }
            if ((h = strchr(s, '%')) != NULL || (h = strchr(s, '@')) != NULL) {
                *h++ = '\0';
                u = s;
            } else if ((u = strrchr(s, '!')) != NULL) {
                *u++ = '\0';
                h = s;
            } else
                h = u = s;
            if (strEQ(u,loginName)) {
                if (in_str(h,hostname,false)) {
...
*/
void
uncache_article (ARTICLE *ap, bool remove_empties)
{
    ARTICLE* next;
...
char *
fetchcache (ART_NUM artnum, int which_line, bool fill_cache)
{
    char* s;
    ARTICLE* ap;
...
*/
char *
get_cached_line (ARTICLE *ap, int which_line, bool no_truncs)
{
    char* s;
    ...
    switch (which_line) {
      ...
      case NGS_LINE:
        if (ap->xrefs && ap->xrefs != nullstr)
            safefree(ap->xrefs);
        if (!strchr(s, ',')) {  /* if no comma, no Xref! */
            safefree(s);
            s = nullstr;
        }
        ap->xrefs = s;
...
bool
art_data (ART_NUM first, ART_NUM last, bool cheating, bool all_articles)
{
    ART_NUM i;
