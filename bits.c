...
rc_to_bits (void)
{
    ...
    for ( ; (c = strchr(s, ',')) != NULL; s = ++c) {    /* for each range */
        ART_NUM min, max;
        *c = '\0';                      /* do not let strchr see past comma */
        h = strchr(s, '-');
        min = atol(s);
...
/*=-=-=-=*/
static int
chase_xref (    /* The Xref-line-using version */
    ART_NUM artnum,
    int markread
)
{
